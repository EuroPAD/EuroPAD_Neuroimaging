...
processing_BIDS_DIR=/scratch/radv/llorenzini/EPAD_dti/raw
OUTPUT_DIR=/scratch/radv/llorenzini/EPAD_dti/derivatives
orig_WORK_DIR=/scratch/radv/llorenzini/EPAD_dti/logs
...
if [[ dtishell=="single" ]]; then 
	...
	RECON_SPEC=$SCRIPTS_DIR/mrtrix_singleshell_ss3t_ACT-fast.json;
	acq="singleshell"
...
if [[ -d $subjectname/$session/dwi ]]; then 
	...
	sbatch $SCRIPTS_DIR/dti_processing_slurm.sh $processing_BIDS_DIR $OUTPUT_DIR $PARTICIPANT_LABEL $WORK_DIR $final_OUTPUT_DIR $session $RECON_SPEC $RUN_DIR;
