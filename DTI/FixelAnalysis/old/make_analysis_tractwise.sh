...
if [[ ! -d ${fixeldir}/tract_fixels/${metric}_smooth ]]; then
	mkdir -p ${fixeldir}/tract_fixels/${metric}_smooth
fi
fixelfilter ${fixeldir}/template/tract_fixels/${metric}/${tract_name} smooth ${fixeldir}/template/tract_fixels/${metric}_smooth/${tract_name} -matrix ${fixeldir}/template/matrix/${tract_name}

if [[ ! -d ${fixeldir}/tract_stats/${metric} ]]; then
	mkdir -p ${fixeldir}/tract_stats/${metric}
fi

fixelcfestats ${fixeldir}/tract_fixels/${metric}_smooth/${tract_name} ${fixeldir}/tractwise_statistics_files/files.txt ${fixeldir}/tractwise_statistics_files/design_matrix.txt ${fixeldir}/tractwise_statistics_files/contrast_matrix.txt ${fixeldir}/matrix/${tract_name} ${fixeldir}/tract_stats/${metric}/${tract_name}
