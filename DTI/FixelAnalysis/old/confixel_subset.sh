...
output=/home/radv/$(whoami)/my-rdisk/r-divi/RNG/Projects/ExploreASL/EPAD/scripts/multimodal_MRI_processing/DTI/FixelAnalysis/subset_837
...
for metric in ${metrics[@]}; do
	for path_to_tract in ${fixeldir}/template/tract_TDIs/*; do #use this for loop if you want to iterate across all tracts
			...
			confixel --index-file $metric/$tract_name/index.mif --directions-file $metric/$tract_name/directions.mif --cohort-file $fixeldir/template/PRS_design_matrices/subset_837/${tract_name}_${metric}_cohort.csv --relative-root $fixeldir/template/tract_fixels --output-hdf5 subset_837_${tract_name}_${metric}.h5
	done
done
