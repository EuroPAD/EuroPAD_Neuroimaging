...
h5_path <- paste0("/scratch/radv/mtranfa/Fixelwise/", tract,  "_fd_smooth.h5")

# create a ModelArray-class object:
# vector_of_analyses <- paste0(prsofint, "_results_lm") 
# modelarray <- ModelArray(h5_path, scalar_types = c("fd_smooth"), vector_of_analyses)
modelarray <- ModelArray(h5_path, scalar_types = c("fd_smooth"))

scalars(modelarray)[["fd_smooth"]]

# filename of example fixel-wise data (.h5 file):
csv <- paste0(tract,  "_fd_smooth_cohort.csv")
phenotypes <- read.csv(file.path(datadir, csv))

EPAD_fixel$source_file <- paste0("fd_smooth/",tract, "/",EPAD_fixel$subject_id, ".mif")
EPAD_fixel$scalar_name <- "fd_smooth"
df_model <- EPAD_fixel[, c("source_file", "scalar_name", prsofint, "prs_onlyapoe", "age_tot", "sex", "PC1", "PC2",
                           "PC3", "PC4", "PC5", "AT", "eTIV_combat_log" )]

for (prs in prsofint){
  if (prs == "prs_apoe"){
    formula.lm = as.formula(paste0("fd_smooth ~" ,prs, "*AT + age_tot + sex + PC1 + PC2 + PC3 + PC4 + PC5"))
    ...
    formula.lm = as.formula(paste0("fd_smooth ~" ,prs, "*AT + prs_onlyapoe + age_tot + sex + PC1 + PC2 + PC3 + PC4 + PC5")) 
    mylm <- ModelArray.lm(formula.lm, 
                          modelarray, 
                          df_model, 
                          "fd_smooth",
