...
metrics=(fd_smooth log_fc_smooth fdc_smooth) #change based on your metrics of interest, options are fd log_fc fdc
...
for metric in ${metrics[@]}; do
	...
	for path_to_tract in ${fixeldir}/template/tract_TDIs/*; do
		...
		for file_path in ${fixeldir}/template/tract_fixels/${metric}/${tract_name}/sub*; do
			file=$(basename $file_path)
			value=$(mrstats ${fixeldir}/template/tract_fixels/${metric}/${tract_name}/${file} -output mean)
