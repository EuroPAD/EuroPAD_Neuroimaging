...
fixeldir=/home/radv/$(whoami)/my-rdisk/r-divi/RNG/Projects/ExploreASL/EPAD/derivatives/DTI_fixels_longitudinal #outpt fixel directory
...
if [ ! -f ${qsirecdir}/${subname}/$sesname/dwi/${subname}_${sesname}group_average_response_wmfod.mif ]; then 
...
###############
for file in $(cat $scriptsdir/subjects_to_be_processed.txt); do echo $file;  session=`basename $file`;  echo $session;  subject=`basename $(dirname $file)`;  echo $subject; cm=" singularity exec  /opt/aumc-containers/singularity/qsiprep/qsiprep-0.19.1.sif ss3t_csd_beta1 ${qsiprepdir}/$subject/$session/dwi/${subject}_${session}_dwi_upsampled.mif ${qsirecdir}/group_average_response_wm.txt  $file/dwi/${subject}_${session}_group_average_response_wmfod.mif ${qsirecdir}/group_average_response_gm.txt  $file/dwi/${subject}_${session}_group_average_response_gmfod.mif ${qsirecdir}/group_average_response_csf.txt  $file/dwi/${subject}_${session}_group_average_response_csffod.mif -mask ${qsiprepdir}/$subject/$session/dwi/${subject}_${session}_mask_upsampled.mif";  echo $cm;  $cm;  done
