...
moving=/home/radv/$(whoami)/my-rdisk/r-divi/RNG/Projects/ExploreASL/AMYPAD/scripts/multimodal_MRI_processing/transforms/Desikan_killiany_MNI152.nii.gz
...
for subjectname in `ls -d ${fmriprep}/sub-* | grep -v html`; do
	...
	if [ `ls -d $subjectname/ses-* | wc -l` -gt 1 ]; then # fmriprep saves differently depending on whether there's one or more sessions
		for session in `ls -d ${subjectname}/ses-*`; do
			...
			output=$fmriprep/$bidsname/anat/${bidsname}_${sesname}_space-T1w_aparcaseg_1mm.nii.gz
		...
		for session in `ls -d ${subjectname}/ses-*`; do
			...
			output=$fmriprep/$bidsname/$sesname/anat/${bidsname}_${sesname}_space-T1w_aparcaseg_1mm.nii.gz
