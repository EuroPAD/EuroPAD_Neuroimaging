...
def extract_timeseries(fmri_file, atlas_file, tr, confounds=None, drop_vols=5, smoothing=0, detrend=False, low_pass=0.1, high_pass=0.01, **kwargs):
    ...
    # Save the post-processed BOLD image before masking (optional output)
    if 'save_processed_bold' in kwargs and kwargs['save_processed_bold']:
        from pathlib import Path
        output_dir = kwargs.get('output_dir', '.')
        output_prefix = kwargs.get('output_prefix', 'processed_bold')
        out_img_path = Path(output_dir) / f"{output_prefix}_bold.nii.gz"
        img.to_filename(str(out_img_path))
    
        # Write a BIDS-style JSON sidecar
        json_path = out_img_path.with_name(out_img_path.name.replace('.nii.gz', '.json'))
        json_content = {
            "Description": "Post-processed BOLD time series after motion regression, smoothing, filtering, and volume trimming.",
            "PostprocessingSteps": {
                "DropVolumes": drop_vols,
                "SmoothingFWHM_mm": smoothing,
                "Denoising": "Confound regression applied" if confounds is not None else "None",
                "ConfoundColumns": kwargs.get("confound_columns", "N/A"),
                "Filtering": {
            ...
            "SourceFile": str(Path(fmri_file).name)
        }
        import json
...
def main():
    ...
    parser.add_argument('--save_processed_bold', action='store_true', help='Save the preprocessed BOLD image before masking')
    ...
    time_series = extract_timeseries(
        ...
        save_processed_bold=args.save_processed_bold,
