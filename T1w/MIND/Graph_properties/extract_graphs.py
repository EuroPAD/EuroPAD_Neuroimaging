...
"""
Script to extract graph properties (as  file)
Connectome input has to be in  format
...
# Define data
ses="ses-01" ## ONLY SESSION ONE FOR NOW
...
netlabels = pd.read_csv("/home/radv/llorenzini/my-rdisk/RNG/Projects/ExploreASL/EPAD/scripts/multimodal_MRI_processing/atlases/Schaeffer_Networks_labels.txt", sep = '\t', header=None)
...
for subpath in data_list:   # Iterate across subjects 
    ...
    if cc:
        ...
        connMat = np.genfromtxt(cc, delimiter=',',  skip_header= True, )
        connMat = np.delete(connMat, 0,1)
#plotting.plot_matrix(connMat, vmax = 1, vmin = -1, colorbar = True)
        ...
        # Matrix Thresholding
        nonbin_connMat=bct.threshold_proportional(connMat, threshold)
        thr_connMat = bct.threshold_proportional(connMat, threshold) # same thing but just different name for further processing
...
#                print(netarea_names) 
        ...
        glob_DF.to_csv(os.path.join(os.path.join(resultdir, subname, ses, subname + "_global_graph_properties"))) # Save as 
        ...
        rsnstrength.to_csv(os.path.join(os.path.join(resultdir, subname, ses, subname + "_RSN_Strengths"))) # Save as 
        ...
        localstrength.to_csv(os.path.join(os.path.join(resultdir, subname, ses, subname + "_local_Strengths"))) # Save as 
        ...
        localbtw.to_csv(os.path.join(os.path.join(resultdir, subname, ses, subname + "_local_Betwennes"))) # Save as 
        ...
        localclust.to_csv(os.path.join(os.path.join(resultdir, subname, ses, subname + "_local_clust"))) # Save as 
        ...
        localpart.to_csv(os.path.join(os.path.join(resultdir, subname, ses, subname + "_local_participation"))) # Save as 
        ...
        richclub.to_csv(os.path.join(os.path.join(resultdir, subname, ses, subname + "_rich_club"))) # Save as 
...
for subname in os.listdir(resultdir): 
    ...
    for sesfold in glob.glob(os.path.join(resultdir, subname, '*ses*')): 
        ...
        ses = os.path.basename(sesfold)  ## Name of Session
        if os.path.exists(os.path.join(resultdir,subname, ses, subname + "_global_graph_properties")):
            # Global metrics  
            globsubdf = pd.read_csv(os.path.join(resultdir,subname, ses, subname + "_global_graph_properties"))
            glob_DF = pd.concat([glob_DF, globsubdf])
            
            # RSN strenght
            rsnstrsubdf = pd.read_csv(os.path.join(resultdir,subname, ses, subname + "_RSN_Strengths"))
            rsnstrength = pd.concat([rsnstrength, rsnstrsubdf])

            # Local strength
            lclstrsubdf = pd.read_csv(os.path.join(resultdir,subname, ses, subname + "_local_Strengths"))
            localstrength = pd.concat([localstrength, lclstrsubdf])

            # Local Betweenness
            btwsubdf = pd.read_csv(os.path.join(resultdir,subname, ses, subname + "_local_Betwennes"))
            localbtw = pd.concat([localbtw, btwsubdf])

            # Local Clustering 
            clstsubdf = pd.read_csv(os.path.join(resultdir,subname, ses, subname + "_local_clust"))
            localclust = pd.concat([localclust, clstsubdf])

            # local Participation Coefficient
            pcsubdf = pd.read_csv(os.path.join(resultdir,subname, ses, subname + "_local_participation"))
            localpart = pd.concat([localpart, pcsubdf])

            # Rich club
            rcsubdf = pd.read_csv(os.path.join(resultdir,subname, ses, subname + "_rich_club"))
            richclub = pd.concat([richclub, rcsubdf])

# Export results

    glob_DF.to_csv(os.path.join(resultdir, "global_graph_properties_" + str(threshold) + "")) # Save as 
    rsnstrength.to_csv(os.path.join(resultdir, "RSN_strengths_" + str(threshold) + ""))
    localstrength.to_csv(os.path.join(resultdir, "local_strength_" + str(threshold) + ""))
    localbtw.to_csv(os.path.join(resultdir, "local_betweenness_" + str(threshold) + ""))
    localclust.to_csv(os.path.join(resultdir, "local_clustering_" + str(threshold) + ""))
    localpart.to_csv(os.path.join(resultdir, "local_participation_" + str(threshold) + ""))
    richclub.to_csv(os.path.join(resultdir, "RichClub_" + str(threshold) + ""))

# for mod in modalities:
#     glob_DF = pd.DataFrame(global_prop[mod])
#     glob_DF.columns = metrics
#     glob_DF.to_csv(os.path.join(resultdir, mod + "_graph_properties")) # Save as 

#     # Regional
#     RSN_strengths_DF = pd.DataFrame(reg_prop[mod + "_RSN_strengths"], columns = np.append("ID", netname))
#     RSN_strengths_DF.to_csv(os.path.join(resultdir, mod + "_RSN_strengths"))
#     strength_DF = pd.DataFrame(reg_prop[mod + "_strength"], columns = np.append("ID", area_names))
#     strength_DF.to_csv(os.path.join(resultdir, mod + "_reg_strength"))
#     btwness_DF = pd.DataFrame(reg_prop[mod + "_btwness"], columns = np.append("ID", area_names))
#     btwness_DF.to_csv(os.path.join(resultdir, mod + "_reg_betweenness"))
#     clust_DF = pd.DataFrame(reg_prop[mod + "_clust"], columns = np.append("ID", area_names))
#     clust_DF.to_csv(os.path.join(resultdir, mod + "_reg_clustering"))
#     part_DF = pd.DataFrame(reg_prop[mod + "_participation"], columns = np.append("ID", area_names))
#     part_DF.to_csv(os.path.join(resultdir, mod + "_reg_participation"))
#     RC_DF = pd.DataFrame(reg_prop[mod + "_richclub"], columns = np.append("ID", richclubnames))
#     RC_DF.to_csv(os.path.join(resultdir, mod + "_RichClub"))
